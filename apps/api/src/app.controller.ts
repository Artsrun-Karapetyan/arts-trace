...
export class AppController {
  ...
  getProjectInvites(@Req() request: AuthedRequest, @Param("id") id: string) {
    return this.appService.getProjectInvites(request.authUser!.id, id);
  }

  @Post("projects/:id/invites")
  createProjectInvite(@Req() request: AuthedRequest, @Param("id") id: string, @Body() body: unknown) {
    return this.appService.createProjectInvite(request.authUser!.id, id, body);
  }

  @Get("invites/:token")
  getInvite(@Param("token") token: string) {
    return this.appService.getInvite(token);
  }

  @Post("invites/:token/accept")
  acceptInvite(@Req() request: AuthedRequest, @Param("token") token: string) {
    return this.appService.acceptInvite(request.authUser!.id, token);
  }
  ...
  addExistingProjectMember(@Req() request: AuthedRequest, @Param("id") id: string, @Body() body: unknown) {
    return this.appService.addExistingProjectMember(request.authUser!.id, id, body);
  }

  @Patch("projects/:projectId/members/:memberId")
  updateProjectMember(@Req() request: AuthedRequest, @Param("projectId") projectId: string, @Param("memberId") memberId: string, @Body() body: unknown) {
    return this.appService.updateProjectMember(request.authUser!.id, projectId, memberId, body);
  }
