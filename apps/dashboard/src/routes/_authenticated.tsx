      ...
      <section className="content-wrap">
        <header className="topbar">
          <div className="topbar-left">
            <button className="btn btn-ghost back-btn" type="button" onClick={goBack}>
...
function ProjectNavigation({ pathname, projectId }: { pathname: string; projectId: string }) {
  return (
    <div className="side-project">
      ...
      <Link className={`nav-link ${pathname.includes("/issues") ? "nav-link-active" : ""}`} to="/projects/$id/issues" params={{ id: projectId }}>
        <svg className="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <Link className={`nav-link ${pathname.includes("/events") ? "nav-link-active" : ""}`} to="/projects/$id/events" params={{ id: projectId }}>
        <svg className="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
